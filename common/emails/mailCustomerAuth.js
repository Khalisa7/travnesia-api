 ...
 */
var RegisUser = function (mailUser, fullName, linkActivated) {
    readHTMLFile(__dirname + '/templates/customers/register.html', function (err, html) {
        ...
        let replacements = {
            ...
            full_name: fullName,
            url_confirmation: process.env.CLIENT_APP_URL + "/confirmation_email" + linkActivated
        }
...
var resetPassFirst = function (mailUser, fullName, linkResetPass) {
    readHTMLFile(__dirname + '/templates/customers/forgot_password_step1.html', function (err, html) {
        let template = handlebars.compile(html)
        ...
        let replacements = {
            ...
            logo: process.env.APP_LOGO,
            full_name: fullName,
            url_forgot_password: process.env.CLIENT_APP_URL + "/reset_pass" + linkResetPass
        }
        htmlToSend = template(replacements)
        mail.send(mailUser, title, text, htmlToSend)
    })
